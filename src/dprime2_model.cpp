...
dprime2_model::dprime2_model(const TGCompositeFrame *MotherFrame, UInt_t w, UInt_t h, covid_viewer *main) : fitmodel(MotherFrame, w, h, main)
{
    fNPars = 5;
    ...
    fHorizontalFrame->AddFrame(label,new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_b2[0] = new TGNumberEntry(fHorizontalFrame, fInitPars[2], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
    fHorizontalFrame->AddFrame(fNE_b2[0],new TGLayoutHints(kLHintsCenterY | kLHintsLeft| kLHintsExpandX,0,0,0,0));
    fHorizontalFrame->AddFrame(new TGLabel(fHorizontalFrame, "Min"),new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_b2[1] = new TGNumberEntry(fHorizontalFrame, fInitLimitsMin[2], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
    fHorizontalFrame->AddFrame(fNE_b2[1],new TGLayoutHints(kLHintsCenterY | kLHintsLeft| kLHintsExpandX,0,0,0,0));
    fHorizontalFrame->AddFrame(new TGLabel(fHorizontalFrame, "Max"),new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_b2[2] = new TGNumberEntry(fHorizontalFrame, fInitLimitsMax[2], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
    ...
    fHorizontalFrame->AddFrame(label,new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_c[0] = new TGNumberEntry(fHorizontalFrame, fInitPars[3], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
    fHorizontalFrame->AddFrame(fNE_c[0],new TGLayoutHints(kLHintsCenterY | kLHintsLeft| kLHintsExpandX,0,0,0,0));
    fHorizontalFrame->AddFrame(new TGLabel(fHorizontalFrame, "Min"),new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_c[1] = new TGNumberEntry(fHorizontalFrame, fInitLimitsMin[3], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
    fHorizontalFrame->AddFrame(fNE_c[1],new TGLayoutHints(kLHintsCenterY | kLHintsLeft| kLHintsExpandX,0,0,0,0));
    fHorizontalFrame->AddFrame(new TGLabel(fHorizontalFrame, "Max"),new TGLayoutHints(kLHintsCenterY | kLHintsLeft, 3, 3, 0, 0));
    fNE_c[2] = new TGNumberEntry(fHorizontalFrame, fInitLimitsMax[3], 7,0, TGNumberFormat::kNESReal, TGNumberFormat::kNEAPositive ,TGNumberFormat::kNELNoLimits);
...
dprime2_model::~dprime2_model()
{

}

void dprime2_model::InitParameters()
{
    TH1 *h = fMainWindow->GetHistoToFit();

    fFitFunction = new TF1(Form("D'2_%s",h->GetName()),this,&dprime2_model::FitFunction,h->GetXaxis()->GetXmin(),h->GetXaxis()->GetXmax(),fNPars,"fitmodel","FitFunction");
    fFitFunction->SetNpx(1000);
    fFitFunction->SetLineColor(kGreen);

    fFitFunction->SetParName(0,"a");
    fFitFunction->SetParName(1,"b1");
    fFitFunction->SetParName(2,"b2");
    fFitFunction->SetParName(3,"c");
    fFitFunction->SetParName(4,"T0");

    fFitFunction->SetParameter(0,fNE_a[0]->GetNumber());
    fFitFunction->SetParameter(1,fNE_b1[0]->GetNumber());
    fFitFunction->SetParameter(2,fNE_b2[0]->GetNumber());
    fFitFunction->SetParameter(3,fNE_c[0]->GetNumber());
    ...
    fFitFunction->SetParameter(4,T0);
    ...
    fFitFunction->SetParLimits(2,fNE_b2[1]->GetNumber(),fNE_b2[2]->GetNumber());
    fFitFunction->SetParLimits(3,fNE_c[1]->GetNumber(),fNE_c[2]->GetNumber());
    fFitFunction->FixParameter(4,T0);
}


Double_t dprime2_model::FitFunction(Double_t*xx,Double_t*pp)
{
    Double_t a1  = pp[0];
    Double_t b1  = pp[1];
    Double_t c1  = pp[3];
