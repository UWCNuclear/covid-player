...
esir_model::esir_model(const TGCompositeFrame *MotherFrame, UInt_t w, UInt_t h, covid_viewer *main) : fitmodel(MotherFrame, w, h, main)
{
    fNPars = 6;
    ...
    fInitLimitsMin[0] = 1e-10;
    fInitLimitsMin[1] = 1.;
    fInitLimitsMin[2] = 1e-10;
    fInitLimitsMin[3] = 1.;
    fInitLimitsMin[4] = 1e-10;

    fInitLimitsMax[0] = 1e-5;
    fInitLimitsMax[1] = 50;
    fInitLimitsMax[2] = 1e-5;
...
esir_model::~esir_model()
{

}

void esir_model::InitParameters()
{
    TH1 *h = fMainWindow->GetHistoToFit();

    fFitFunction = new TF1(Form("ESIR_%s",h->GetName()),this,&esir_model::FitFunction,h->GetXaxis()->GetXmin(),h->GetXaxis()->GetXmax(),fNPars,"fitmodel","FitFunction");
...
Double_t esir_model::FitFunction(Double_t*xx,Double_t*pp)
{
    Double_t a  = pp[0];
    Double_t b  = pp[1];
    Double_t c  = pp[2];
