...
dprime2full_model::dprime2full_model(const TGCompositeFrame *MotherFrame, UInt_t w, UInt_t h, covid_viewer *main) : fitmodel(MotherFrame, w, h, main)
{
    fNPars = 7;
...
dprime2full_model::~dprime2full_model()
{

}

void dprime2full_model::InitParameters()
{
    TH1 *h = fMainWindow->GetHistoToFit();

    fFitFunction = new TF1(Form("D'2Full_%s",h->GetName()),this,&dprime2full_model::FitFunction,h->GetXaxis()->GetXmin(),h->GetXaxis()->GetXmax(),fNPars,"fitmodel","FitFunction");
...
Double_t dprime2full_model::FitFunction(Double_t*xx,Double_t*pp)
{
    Double_t a1  = pp[0];
    Double_t b1  = pp[1];
    Double_t c1  = pp[2];
