...
d2_model::d2_model(const TGCompositeFrame *MotherFrame, UInt_t w, UInt_t h, covid_viewer *main) : fitmodel(MotherFrame, w, h, main)
{
    fNPars = 5;
...
void d2_model::InitParameters()
{
    TH1 *h = fMainWindow->GetHistoToFit();

    fFitFunction = new TF1(Form("D'2_%s",h->GetName()),this,&d2_model::FitFunction,h->GetXaxis()->GetXmin(),h->GetXaxis()->GetXmax(),fNPars,"fitmodel","FitFunction");
...
Double_t d2_model::FitFunction(Double_t*xx,Double_t*pp)
{
    Double_t a1  = pp[0];
    Double_t b1  = pp[1];
    Double_t b2  = pp[2];
